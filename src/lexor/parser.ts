...
export function getCandidateDicPaths(): string[] {
    ...
    if (typeof window !== 'undefined' && typeof location !== 'undefined') {
        ...
        candidates.push('./kuromoji-dict/');
        // origin-prefixed
        try { candidates.push(location.origin + '/kuromoji-dict/'); } catch (e) { /* ignore */ }
    }
...
export async function initKuromoji(): Promise<void> {
    ...
    for (const dicPath of candidates) {
        ...
        const isRealBrowser = (typeof fetch !== 'undefined'
        ...
        if (typeof window === 'undefined') {
            try {
                ...
                const probeFs = dicPath.endsWith('/') ? path.join(dicPath, 'base.dat.gz') : path.join(dicPath, 'base.dat.gz');
            ...
            try {
                kuromoji.builder({ dicPath }).build((err: Error | null, tk: any) => {
                    if (err) {
                        ...
                        try { lastBuildErrorDetail = JSON.stringify(eAny); } catch (_) { lastBuildErrorDetail = String(eAny); }
    ...
    console.warn('kuromoji build failed for all candidate dicPaths, falling back to simple tokenizer', candidates);
    tokenizer = null;
}
